...
// For server-side rendering (Next.js server), use internal Docker URL
// For client-side (browser), use public URL or empty for same-origin relative paths
const STRAPI_INTERNAL_URL = process.env.STRAPI_INTERNAL_URL || '';
const STRAPI_PUBLIC_URL = process.env.NEXT_PUBLIC_STRAPI_URL || '';

// Determine which URL to use based on execution context
const isServer = typeof window === 'undefined';
const STRAPI_URL = isServer && STRAPI_INTERNAL_URL ? STRAPI_INTERNAL_URL : STRAPI_PUBLIC_URL;
